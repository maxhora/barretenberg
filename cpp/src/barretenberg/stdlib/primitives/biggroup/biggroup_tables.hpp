...
element<C, Fq, Fr, G> element<C, Fq, Fr, G>::eight_bit_fixed_base_table<X>::operator[](const field_t<C>& index) const
{
    ...
    const auto xlo = plookup_<C>::read_pair_from_table(tags[0], index);
    const auto xhi = plookup_<C>::read_pair_from_table(tags[1], index);
    const auto ylo = plookup_<C>::read_pair_from_table(tags[2], index);
    const auto yhi = plookup_<C>::read_pair_from_table(tags[3], index);
    const auto xyprime = plookup_<C>::read_pair_from_table(tags[4], index);
